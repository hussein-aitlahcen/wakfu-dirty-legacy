...
public class FightMap extends TopologyMapInstanceSet
{
    ...
    public boolean isMovementBlocked(final int x, final int y, final short z) {
        ...
        for (final Object obstacleId : obstaclesId) {
            final FightObstacle obstacle = this.m_obstacles[(Byte)obstacleId];
            if (obstacle == null) {
                return false;
            }
            final boolean movementBlocked = obstacle.isBlockingMovement();
            if (movementBlocked && !this.m_ignoredMovementObstacles.contains((Byte)obstacleId)) {
                return true;
            }
        }
        return false;
    ...
    public boolean isSightBlocked(final int x, final int y, final short z) {
        ...
        for (final Object obstacleId : obstaclesId) {
            ...
            if (sightBlocked && !this.m_ignoredSightObstacles.contains((Byte)obstacleId)) {
                return true;
            }
        }
        return false;
