...
public class CharacterSerializedBags extends CharacterSerializedPart implements VersionableObject
{
    public final RawBagContainer bagContainer;
    private final BinarSerialPart m_binarPart;
    
    public CharacterSerializedBags() {
        super();
        this.bagContainer = new RawBagContainer();
        this.m_binarPart = new BinarSerialPart() {
            @Override
            public void serialize(final ByteBuffer buffer) {
                try {
                    final boolean ok = CharacterSerializedBags.this.serialize(buffer);
                    if (ok) {
                        this.markAsSuccess();
                    }
                    else {
                        this.markAsError("Erreur lors de la s\u00e9rialisation de CharacterSerializedBags");
                    }
                }
                catch (Exception e) {
                    this.markAsError("Exception lev\u00e9e lors de la s\u00e9rialisation de CharacterSerializedBags", e);
                }
            }
            ...
            public void unserialize(final ByteBuffer buffer, final int version) {
                try {
                    final boolean ok = CharacterSerializedBags.this.unserializeVersion(buffer, version);
                    if (ok) {
                        this.markAsSuccess();
                    }
                    else {
                        this.markAsError("Erreur lors de la d\u00e9s\u00e9rialisation de CharacterSerializedBags");
                    }
                }
                catch (Exception e) {
                    this.markAsError("Exception lev\u00e9e lors de la d\u00e9s\u00e9rialisation de CharacterSerializedBags", e);
                }
            }
            
            @Override
            public int expectedSize() {
                return CharacterSerializedBags.this.serializedSize();
            }
        };
    }
    ...
    public boolean serialize(final ByteBuffer buffer) {
        final boolean bagContainer_ok = this.bagContainer.serialize(buffer);
        return bagContainer_ok;
    }
    
    @Override
    public boolean unserialize(final ByteBuffer buffer) {
        final boolean bagContainer_ok = this.bagContainer.unserialize(buffer);
        return bagContainer_ok;
    }
    
    @Override
    public void clear() {
        this.bagContainer.clear();
    }
    ...
    public boolean unserializeVersion(final ByteBuffer buffer, final int version) {
        ...
        final CharacterSerializedBagsConverter converter = new CharacterSerializedBagsConverter();
    ...
    public int serializedSize() {
        int size = 0;
        size += this.bagContainer.serializedSize();
        return size;
    }
    ...
    public final void internalToString(final StringBuilder repr, final String prefix) {
        repr.append(prefix).append("bagContainer=...\n");
        this.bagContainer.internalToString(repr, prefix + "  ");
    }
    
    private final class CharacterSerializedBagsConverter
    {
        private final RawBagContainer bagContainer;
        
        private CharacterSerializedBagsConverter() {
            super();
            this.bagContainer = new RawBagContainer();
        }
        
        public void pushResult() {
            CharacterSerializedBags.this.bagContainer.bags.clear();
            CharacterSerializedBags.this.bagContainer.bags.ensureCapacity(this.bagContainer.bags.size());
            CharacterSerializedBags.this.bagContainer.bags.addAll(this.bagContainer.bags);
        }
        ...
        private boolean unserialize_v1(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 1);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v313(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 313);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v314(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 314);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v315(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 315);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10003(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10003);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10020(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10020);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10023(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10023);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10028000(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10028000);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10029000(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10029000);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10032003(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10032003);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10035004(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10035004);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10035007(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10035007);
            return bagContainer_ok;
        }
        
        private boolean unserialize_v10036004(final ByteBuffer buffer) {
            final boolean bagContainer_ok = this.bagContainer.unserializeVersion(buffer, 10036004);
            return bagContainer_ok;
        }
