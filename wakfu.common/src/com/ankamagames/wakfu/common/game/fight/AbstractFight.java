...
public abstract class AbstractFight<F extends BasicCharacterInfo> extends WakfuTurnBasedFight<F> implements TimeEventHandler
{
    ...
    public void updateAggroFromEffect(final WakfuRunningEffect effect) {
        if ((effect).getGenericEffect() != null && (effect).getGenericEffect().getEffectType() != 2) {
            return;
        }
        if ((effect).getEffectContainer() != null) {
            final short aggroWeight = (effect).getEffectContainer().getAggroWeight();
            final short allyEfficacity = (effect).getEffectContainer().getAllyEfficacity();
            final short foeEfficacity = (effect).getEffectContainer().getFoeEfficacity();
            EffectUser caster = effect.getCaster();
            if ((effect).getEffectContainer().getContainerType() == 3) {
                caster = ((BasicEffectArea<WakfuEffect, EffectAreaParameters>) (effect).getEffectContainer()).getOwner();
            }
