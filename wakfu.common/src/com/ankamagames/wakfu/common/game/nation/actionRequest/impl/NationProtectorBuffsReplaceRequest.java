...
public class NationProtectorBuffsReplaceRequest extends NationActionRequest
{
    ...
    public NationProtectorBuffsReplaceRequest() {
        super(NationActionRequestType.PROTECTOR_BUFFS_REPLACE);
    }
    ...
    public void execute() {
        ...
        if (nation == null) {
            NationProtectorBuffsReplaceRequest.m_logger.error("Impossible d'ex\u00e9cuter l'action " + this + " : la nation " + this.m_nationId + " n'existe pas");
            return;
        }
        nation.requestReplaceProtectorBuffs(this.m_protectorId, this.m_buffs);
    }
    ...
    public boolean unserialize(final ByteBuffer buffer) {
        ...
        if (remainingBytes % 4 != 0) {
            NationProtectorBuffsReplaceRequest.m_logger.error("Impossible de d\u00e9s\u00e9rialiser une nationActionRequest. Nombre d'octets restants non valides pour former des entiers... pas multiple de 4.");
            return false;
        }
    ...
    static {
        m_logger = Logger.getLogger(NationProtectorBuffsReplaceRequest.class);
        FACTORY = new NationActionRequestFactory() {
            @Override
            public NationActionRequest createNew() {
                return new NationProtectorBuffsReplaceRequest();
            }
        };
    }
}
