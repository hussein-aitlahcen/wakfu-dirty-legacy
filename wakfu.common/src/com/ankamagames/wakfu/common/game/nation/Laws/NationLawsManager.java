...
public class NationLawsManager
{
    ...
    public ArrayList<NationLaw> getTriggeringLaws(final NationLawEvent event) {
        ...
        for (final Object lawId : laws) {
            if (!this.m_activeLaws.contains((long) lawId)) {
                continue;
            }
            final NationLaw law = this.m_laws.get((long) lawId);
    ...
    public int getActivationCost(final TLongHashSet laws) {
        int cost = 0;
        final TLongIterator it = (TLongIterator) laws.iterator();
        while (it.hasNext()) {
    ...
    public void toRaw(final RawNationLaw raw) {
        ...
        final TLongIterator it = (TLongIterator) this.m_activeLaws.iterator();
        while (it.hasNext()) {
