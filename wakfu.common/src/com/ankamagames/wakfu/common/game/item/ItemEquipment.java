...
public class ItemEquipment extends ArrayInventoryWithoutCheck<Item, RawInventoryItem> implements RawConvertible<RawInventoryItemInventory>, LoggableEntity
{
    ...
    public ItemEquipment() {
        super(Item.class, EquipmentItemProvider.getInstance(), EquipmentInventoryChecker.getInstance(), EquipmentPosition.getEquipmentSize(), false, false);
    ...
    public final boolean toRaw(final RawInventoryItemInventory raw) {
        if (this.m_serializeQuantity) {
            ItemEquipment.m_logger.warn("Impossible d'ajouter l'information de quantit\u00e9 \u00e0 un RawInventoryItemInventory qui n'est pas pr\u00e9vu pour");
        }
        raw.clear();
        ...
        while (it.hasNext()) {
            ...
            try {
                final Item item = this.m_contents[pos];
                ...
                final Object content2 = this.m_contents[pos];
                ItemEquipment.m_logger.error("erreur de cast : content is " + content2.getClass().getName(), e);
            }
            catch (Exception e2) {
                ItemEquipment.m_logger.error("Exception lev\u00e9e lors de la r\u00e9cup\u00e9ration de l'item \u00e0 la position " + pos + " de l'\u00e9quipement " + this, e2);
            }
        }
        return true;
    }
    ...
    public boolean fromRaw(final RawInventoryItemInventory raw) {
        try {
            this.destroyAll();
            if (this.m_serializeQuantity) {
                ItemEquipment.m_logger.warn("Impossible de d\u00e9s\u00e9rialiser la quantit\u00e9 d'objets dans un inventaire, cette information n'est pas pr\u00e9sente dans le format RawInventory");
            }
            boolean valid = true;
            for (final RawInventoryItemInventory.Content content : raw.contents) {
                try {
                    final Item item = this.m_contentProvider.unSerializeContent(content.item);
                    if (item != null) {
                        ...
                        valid = false;
                        ItemEquipment.m_logger.error("Erreur lors de la d\u00e9-serialisation d'un ArrayInventory : item null");
                    }
                }
                catch (InventoryCapacityReachedException e) {
                    ItemEquipment.m_logger.error(ExceptionFormatter.toString(e));
                    valid = false;
                }
                catch (ContentAlreadyPresentException e2) {
                    ItemEquipment.m_logger.error(ExceptionFormatter.toString(e2));
                    valid = false;
                }
                catch (PositionAlreadyUsedException e3) {
                    ItemEquipment.m_logger.error(ExceptionFormatter.toString(e3));
                    valid = false;
                }
                catch (Exception e4) {
                    ItemEquipment.m_logger.error(ExceptionFormatter.toString(e4));
                    valid = false;
                }
            }
            return valid;
            ...
            ItemEquipment.m_logger.error("Impossible d'initialiser un " + this.getClass() + " \u00e0 partir d'un " + raw.getClass() + " : RawArrayInventory attendu", e5);
            return false;
        }
    }
    ...
    static {
        m_logger = Logger.getLogger(ItemEquipment.class);
    }
}
