...
public final class DoubleOrQuits extends WakfuRunningEffect
{
    ...
    public DoubleOrQuits newInstance() {
        ...
        try {
            ...
            re.m_pool = null;
            re.m_isStatic = false;
            RunningEffect.m_logger.error("Erreur lors d'un checkOut sur un DoubleOrQuits : " + e.getMessage());
        }
    ...
    protected void executeOverride(final RunningEffect triggerRE, final boolean trigger) {
        this.setNotified();
        if (triggerRE == null) {
            RunningEffect.m_logger.error("Ne peut \u00eatre ex\u00e9cut\u00e9 que via trigger");
            return;
        }
        if (triggerRE.getGenericEffect() == null) {
            RunningEffect.m_logger.error("Generic effect inconnu, on ne peut pas faire les test n\u00e9cessaires pour executer l'effet");
            return;
        }
        final BitSet additionalTriggers = triggerRE.getGenericEffect().getExecutionTriggersAdditionnal();
        if (additionalTriggers == null) {
            RunningEffect.m_logger.error("Trigger propres du declencheurs inconnus, on ne peut pas faire les test n\u00e9cessaires pour executer l'effet");
            return;
        }
    ...
    private void activate() {
        ...
        if (dealDamage) {
            ...
            final HPLoss damage = HPLoss.checkOut(this.m_context, this.m_element, HPLoss.ComputeMode.CLASSIC, value, this.m_target);
            damage.setCaster(this.m_caster);
            damage.setGenericEffect(DefaultFightInstantEffectWithChatNotif.getInstance());
            ...
            final HPGain heal = HPGain.checkOut(this.m_context, this.m_element);
            heal.setTarget(this.m_target);
            heal.setCaster(this.m_caster);
            heal.forceValue(this.m_value);
            heal.setGenericEffect(DefaultFightInstantEffectWithChatNotif.getInstance());
    ...
    private void record(final RunningEffect triggerRE) {
        if (triggerRE == null) {
            RunningEffect.m_logger.error("Pas d'effet declencheur pour enregistrer la valeur");
            return;
        }
