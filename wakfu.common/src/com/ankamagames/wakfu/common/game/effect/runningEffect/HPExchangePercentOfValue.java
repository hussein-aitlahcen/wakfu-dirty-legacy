...
public class HPExchangePercentOfValue extends WakfuRunningEffect
{
    ...
    protected void executeOverride(final RunningEffect linkedRE, final boolean trigger) {
        if (this.isValueComputationEnabled() && this.m_target != null && this.m_target.hasCharacteristic(FighterCharacteristicType.HP)) {
            ...
            HPLoss re2 = HPLoss.checkOut(this.m_context, Elements.PHYSICAL, HPLoss.ComputeMode.CLASSIC, this.m_value, this.m_caster);
            re2.setTarget(this.m_caster);
            re2.forceValue(this.m_value);
            re2.execute(null, false);
            HPGain re3 = HPGain.checkOut(this.m_context, Elements.PHYSICAL);
            re3.setTarget(this.m_target);
            re3.forceValue(this.m_value);
            re3.execute(null, false);
        }
        else {
    ...
    public void effectiveComputeValue(final RunningEffect triggerRE) {
        ...
        if (this.m_genericEffect != null) {
            this.m_percent = this.m_genericEffect.getParam(0, level, RoundingMethod.RANDOM);
        }
    }
