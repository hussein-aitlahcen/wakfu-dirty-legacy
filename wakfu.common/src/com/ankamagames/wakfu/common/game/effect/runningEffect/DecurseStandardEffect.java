...
public class DecurseStandardEffect extends WakfuRunningEffect
{
    ...
    protected void executeOverride(final RunningEffect linkedRE, final boolean trigger) {
        ...
        for (final RunningEffect re : this.m_target.getRunningEffectManager()) {
            ...
            if (removeByActionID) {
                ...
                effectId = re.getGenericEffect().getEffectId();
            }
            if (re.getGenericEffect() != null && effectToRemoveId == effectId) {
    ...
    private boolean shouldDecurse(final RunningEffect<?, ?> re) {
        ...
        if (isFightEffect) {
            final boolean isDecursable = ((State) re.getGenericEffect()).isDecursable();
            if (!isDecursable) {
                return false;
            }
        }
