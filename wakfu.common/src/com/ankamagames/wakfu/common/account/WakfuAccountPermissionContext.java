        ...
        public boolean hasPermission(final WakfuAccountInformationHolder... holders) {
            for (int i = 0, size = holders.length; i < size; ++i) {
                ...
                if (!handler.isDebuggingHackMode()) {
                    return handler.getActiveSubscriptionLevel() == SubscriptionLevel.EU_SUBSCRIBER || (handler.getActiveSubscriptionLevel() != SubscriptionLevel.EU_FREE && WakfuAccountPermissionContext.SUBSCRIBER_ZONE.hasPermission(holder));
                }
            }
            return true;
        }
