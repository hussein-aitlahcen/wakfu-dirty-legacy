...
public class ReferenceResource extends AbstractReferenceResource<ResourceEvolutionStep>
{
    ...
    private void buildGfxPaths(final TIntObjectHashMap<int[]> gfxIds) {
        try {
            ...
            ReferenceResource.m_logger.error("Erreur \u00e0 la r\u00e9cup\u00e9ration du chemin des anms de ressources", e);
        }
    }
    
    public String getGfx(final int worldX, final int worldY, final int worldId) {
        if (this.m_gfxPaths.isEmpty()) {
            ReferenceResource.m_logger.error("La resource " + this.getId() + " n'a pas de gfxId");
            return null;
        }
        ...
        if (gfxPaths == null) {
            ...
            if (gfxPaths == null) {
                ReferenceResource.m_logger.error("Pas de gfxIds par d\u00e9faut pour la resource " + this.getId());
                return null;
            }
        }
    ...
    static {
        m_logger = Logger.getLogger(ReferenceResource.class);
    }
}
