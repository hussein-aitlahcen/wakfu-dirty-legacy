...
public class SpellManager extends AbstractSpellManager<Spell>
{
    ...
    public Spell getSpell(final int spellId) {
        ...
        Spell spell = this.m_spells.get(spellId);
    ...
    public static SpellManager getInstance() {
        if (AbstractSpellManager.m_instance == null) {
            AbstractSpellManager.m_instance = new SpellManager();
        }
        return (SpellManager)AbstractSpellManager.m_instance;
    }
    ...
    public static SpellParameters buildSpellParameters(final SpellBinaryData bs) {
        ...
        try {
            ...
            AbstractSpellManager.m_logger.error("Erreur lors de la compilation des crit\u00e8res du sort " + bs.getId(), e);
        }
        ...
        try {
            ...
            AbstractSpellManager.m_logger.error("Erreur lors de la compilation des crit\u00e8res d'apprentissage du sort " + bs.getId(), e2);
        }
        ...
        if (alternativeCasts != null && alternativeCasts.size() != 0) {
            alternativeCasts.forEachEntry(new TObjectObjectProcedure<String, SpellBinaryData.CastParam>() {
                ...
                public boolean execute(final String criterion, final SpellBinaryData.CastParam param) {
                    try {
                        final SimpleCriterion alternativeCastCriterion = null;//CriteriaCompiler.compileBoolean(criterion);
                        spellParameters.addAlternativeCastParameters(alternativeCastCriterion, convert(param));
                    }
                    catch (Exception e) {
                        AbstractSpellManager.m_logger.error("Erreur lors de la compilation des crit\u00e8res d'apprentissage du sort " + bs.getId(), e);
                    }
                    return true;
                }
            });
    ...
    public static void addEffects(final AbstractSpell spell, final int[] effectIds, final EffectManager effectManager) {
        for (final int effectId : effectIds) {
            ...
            if (effect != null) {
                ...
                AbstractSpellManager.m_logger.error("Probl\u00e8me de chargmeent de Spell " + spell.getId());
            }
        }
    }
}
