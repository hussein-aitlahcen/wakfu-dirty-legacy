...
public class TLongArrayList implements Externalizable, Cloneable
{
    ...
    public long getSet(final int offset, final long val) {
        if (offset >= this._pos) {
            throw new ArrayIndexOutOfBoundsException(offset);
        }
    ...
    public long remove(final int offset) {
    ...
    public TLongArrayList subList(final int begin, final int end) {
        if (end < begin) {
    ...
    public long[] toNativeArray(final int offset, final int len) {
        final long[] rv = new long[len];
        this.toNativeArray(rv, offset, len);
        return rv;
    }
    
    public void toNativeArray(final long[] dest, final int offset, final int len) {
        if (len == 0) {
            return;
        }
        if (offset < 0 || offset >= this._pos) {
    ...
    public TLongArrayList grep(final TLongProcedure condition) {
    ...
    public TLongArrayList inverseGrep(final TLongProcedure condition) {
        final TLongArrayList list = new TLongArrayList();
        for (int i = 0; i < this._pos; ++i) {
            if (!condition.execute(this._data[i])) {
                list.add(this._data[i]);
