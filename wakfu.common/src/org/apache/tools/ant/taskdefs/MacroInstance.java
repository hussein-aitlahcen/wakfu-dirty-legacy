...
public class MacroInstance extends Task implements DynamicAttribute, TaskContainer
{
    ...
    private void processTasks() {
        ...
        for (final Task ue : this.unknownElements) {
            final String name = ProjectHelper.extractNameFromComponentName(((UnknownElement) ue).getTag()).toLowerCase(Locale.ENGLISH);
            ...
            this.presentElements.put(name, (UnknownElement) ue);
        }
    }
    ...
    private UnknownElement copy(final UnknownElement ue, final boolean nested) {
        ...
        for (final Map.Entry<String, Object> entry : m.entrySet()) {
            rc.setAttribute(entry.getKey(), this.macroSubs((String) entry.getValue(), this.localAttributes));
        }
        ...
        while (e.hasMoreElements()) {
            ...
            if (templateElement == null || nested) {
                ...
                while (i.hasNext()) {
                    final UnknownElement child2 = this.copy((UnknownElement) i.next(), true);
