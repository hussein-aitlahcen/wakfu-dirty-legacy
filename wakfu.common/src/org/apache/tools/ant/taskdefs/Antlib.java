...
public class Antlib extends Task implements TaskContainer
{
    ...
    public void execute() {
        for (final Object ue : this.tasks) {
            this.setLocation(((ProjectComponent) ue).getLocation());
            ((Task) ue).maybeConfigure();
            final Object configuredObject = ((UnknownElement) ue).getRealThing();
            if (configuredObject == null) {
                continue;
            }
            if (!(configuredObject instanceof AntlibDefinition)) {
                throw new BuildException("Invalid task in antlib " + ((UnknownElement) ue).getTag() + " " + configuredObject.getClass() + " does not " + "extend org.apache.tools.ant.taskdefs.AntlibDefinition");
            }
