...
import com.ankamagames.wakfu.common.game.respawn.RespawnPointHandler;
import com.ankamagames.wakfu.common.game.travel.TravelInfoManager;
...
abstract class AbstractPlayerCharacter extends AbstractCharacterInfo {
    ...
    public void setNation(int nationId) {
    	m_nationId = nationId;
    }
...
//	            AbstractPlayerCharacter.this.m_dndStateActive = this.m_part.dndState;
	    ...
	    private final class PlayerCharacterPartAchievements extends CharacterInfoPart
	    {
	        ...
	        public void updateToSerializedPart() {
	        	m_part.serializedAchievementsContext = new byte[4 + 1 + 4 + 4 + 4 + 1];
	        	ByteArray array = ByteArray.wrap(m_part.serializedAchievementsContext);
	        	array.putInt(0); // version
	        	array.put((byte)0); // history
	        	array.putInt(0);// nbVariables
	        	array.putInt(0); // nbAchievements
	        	array.putInt(0);; // nbObjectives
	        	array.put((byte)0); // numFollowed
	        	m_logger.info("achievements created");
	        }
...
//	            AbstractPlayerCharacter.this.getCharacteristics().fromRaw(this.m_part.characteristics);
	    ...
	    private final class LocalPlayerCharacterPartDimensionalBagForClient extends CharacterInfoPart
	    {
	        ...
	        public void updateToSerializedPart() {
	        	m_part.bag.ownerName = m_name;
	        	m_part.bag.ownerId = m_id;
	        	m_part.bag.customViewModelId = 408;
	        	m_part.bag.permissions.dimensionalBagLocked = false;
...
//	            comportment.setDailyPvpMoneyAmount(this.m_part.dailyPvpMoneyAmount);
	    ...
	    private final class PlayerCharacterPartPersonalEffects extends CharacterInfoPart
	    {
	        ...
	        public void updateToSerializedPart() {
	        	m_part.antiAddictionEffects = new int[0];
	        	m_part.guildEffects = new int[0];
	        	m_part.havenWorldEffects = new int[0];
	        }
