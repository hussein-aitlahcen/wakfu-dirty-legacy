...
import com.akfu.common.network.protocol.message.clientToServer.ClientDispatchAuthenticationMessage
...
object AuthenticationFrame extends FrameBase[AuthClient, WakfuClientMessage] {  
  ...
  def handleClientAuthentication(client: AuthClient, message: ClientDispatchAuthenticationMessage) {
    ...
    client.getWorker ! Authentication(client, new String(rawAcc), new String(rawPass))
  } 
}
